...
rbtree_node *rbtree_create_header(rbtree_node *root) {
	global_header = malloc(sizeof(rbtree_node));
	global_header->right = root;
	global_header->left = root;
	global_header->parent = root;
...
rbtree_node *rbtree_create_root(int key, char *value) {
	...
	node->left = NULL;
	node->right = NULL;
	node->parent = rbtree_create_header(node);
	node->value = strdup(value);
	node->color = BLACK;
	global_root = node;
