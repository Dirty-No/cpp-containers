...
// 	{
// 		delete_rbtree_node(N);
// 		global_root = NULL;
// 	}

// 	// N has two non nil child
// 	if (N->left && N->right) {
// 		rbtree_node *R = rbtree_find_inorder_predecessor(N);
// 		rbtree_swap_nodes(N, R);
// 	}
// }	

rbtree_node * rbtree_find_inorder_successor(rbtree_node *pos) {
	pos = pos->right;
	while (pos->left)
		pos = pos->left;
	return pos;
...
void rbtree_fix_double_black(rbtree_node *v) {

}

rbtree_node *rbtree_erase(rbtree_node *root, int key) {
	rbtree_node *K = rbtree_find(root, key);

	if (K == NULL)
		return NULL;

	rbtree_node * K_left = K->left;
	rbtree_node * K_right = K->right;
	const color K_color = K->color;
	rbtree_node *P = K->parent;

	delete_rbtree_node(K);

	// No child, deleted node is a leaf
	if (K_left == NULL || K_right == NULL) {


		// Ensure K had a parent
		if (P == NULL)
		{
			// We've deleted root
			global_root = NULL;
	...
	// One child (so K was black)
	if ((K_left != NULL) ^ (K_right != NULL)) {
		// 1 2 3 4 5 6 pics

		// We know K as a child
		rbtree_node * K_child = K_left ? K_left : K_right;
		...
		// We now know K had a parent
		const int isLeft = P->left == K;

		// K's child takes it's place
		if (isLeft)
			P->left = K_left ? K_left : K_right;
		else
			P->right = K_left ? K_left : K_right;
		
		// K was black, so its replacement takes its color
		K_child->color = BLACK;

		// We're done for thoses cases
		return K_child;
	}

	// Two childs (hard part that requires fixing and recursion and fucking shit going around)
